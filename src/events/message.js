...
client.on('message', (msg) => {
    (async () => {
        ...
        for (let i = 0; i < msg.attachments.size; i++) {
            https.get(msg.attachments.array()[i].proxyURL, (resp) => Logger.log('GET Completed', 'INFO')).on('error', (err) => Logger.handleError(err)); // GET to cache content but ignore response.
        }
